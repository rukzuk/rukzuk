 ...
 *
 *  @history
 *    2013-02-08 kgl many many changes and bug fixes
...
Ext.ux.form.MultiSelect = Ext.extend(Ext.form.Field, {
    ...
    valueField: 1,
    /**
     * @cfg {String/Number} hierarchy Name/Index of the desired hierarchy field in the dataset (defaults to 2).
     */
    hierarchyField: 2,
    /**
    ...
    /**
     * @cfg {Boolean} multiSelect False to allow only a single element to be selected (defaults to true).
     */
    multiSelect: true,
    ...
    // internal representation of our values
    valueArray: [],
    ...
    initComponent: function () {
        ...
        if (Ext.isArray(this.store)) {
            ...
            this.hierarchyField = 'hierarchy';
        } else if (this.store) {
            this.store = Ext.StoreMgr.lookup(this.store);
        } else {
            throw 'MultiSelect needs a store';
        }
        ...
        // create initial version of internal representation of our values (convert string to array)
        if (this.value) {
    ...
    onRender: function (ct, position) {
        ...
        var fs = this.fs = new Ext.form.FieldSet({
            ...
            title: this.legend,
            height: this.height,
            bodyStyle: 'height:' + this.height + 'px;',
            width: this.width,
            style: 'padding:0; ',
            tbar: this.tbar
        });
            ...
            columns: [
                {
                    ...
                    // custom template for hierarchy fields
                    tpl: '<span class="ux-mselect-indent ux-mselect-indent-l{' + this.hierarchyField + '}">{' + this.displayField + '}</span>'
                }
            ],
            hideHeaders: true
        });
    ...
    // private
    onViewSelectionChange: function (vw, selectedEl) {

        // update internal values array (on change of user interface)
        var values = [];
        var selectionsArray = this.view.getSelectedIndexes();

        for (var i = 0; i < selectionsArray.length; i++) {
            values.push(this.store.getAt(selectionsArray[i]).get(this.valueField));
        }

        console.log('[MultiSelect] onViewSelectionChange', ' onViewSelectionChange ', values);
     ...
     * by {@link #delimiter}.
     * @return {String} value An string of data values separated by {@link #delimiter}
     */
    getValue: function () {
    ...
    setValue: function (values) {
        ...
        if (!Ext.isArray(values)) {
            // split by delimiter if multiSelect is on
            if (this.multiSelect === true) {
                values = String(values).split(this.delimiter);
            } else {
                // use values as whole string if multiSelect is disabled
                values = ['' + values];
            }
        }
        // set value to internal variable
        this.valueArray = values;
        this.value = this.valueArray.join(this.delimiter);


        // update list view & update hidden field
        if (this.rendered) {
            this.updateListView();
        ...
        Ext.ux.form.MultiSelect.superclass.setValue.call(this, this.value);
    ...
    /**
     * update the list and select the correct list items based on {@Link #values}
     * does not fire any change events!
     */
    updateListView: function () {
        ...
        for (var i = 0; i < values.length; i++) {
            console.log('[MultiSelect] updateListView id: ', values[i], ' index: ', this.view.store.indexOfId(values[i]));
            selections.push(this.view.store.indexOfId(values[i]));
        }
        this.view.select(selections, false, true);
    },

    /**
     * updates the hidden field (used for form submit)
     */
