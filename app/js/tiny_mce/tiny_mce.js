...
/*
	disabled DOM.hide()/DOM.show() because it adds display:block to all editables
*/
			//DOM.hide(e);
			...
			t.onInit.dispatch(t);
			t.focus(true);
			//t.nodeChanged({initial : 1}); // removed by Seitenbau: nodeChange event fires anyway
		...
		nodeChanged : function(o) {
			...
			if (t.initialized) {
				...
				t.dom.getParent(n, function(node) {
					if (node.nodeName == 'BODY' || node == el) //added by Seitenbau: stop when editor coontainer element is reached
						return true;
		...
		remove : function() {
            ...
            //added by Seitenbau: disable contentEditable if set
            var element = t.getElement();
            if (element && t.settings.content_editable) {
		...
		_addEvents : function() {
			...
			each(lo, function(v, k) {
				switch (k) {
					...
					case 'focus':
					case 'blur':
						dom.bind(s.content_editable ? t.getBody() : t.getWin(), k, eventHandler);
						break;
...
(function(tinymce) {
	...
	function splitList(selection, dom, li) {
		...
		if (dom.isEmpty(li) || li.innerHTML.toLowerCase() == '<br>') { // HACK: dom.isEmpty() returns false if there is only an BR inside element (SBCMS-629 / http://www.tinymce.com/develop/bugtracker_view.php?id=4778)
		...
		init : function(ed, url) {
			...
			function grabContent(e) {
				...
				dom.setStyles(n, {
					...
					top : tinymce.isWebKit ? 0 : posY - 25,
					width : 1,
					height : 1,
					overflow : 'hidden'
				});
			...
			if (getParam(ed, "paste_auto_cleanup_on_paste")) {
                ...
                //added by Seitenbau: removed check for Firefox 2 since it breaks paste in Firefox > 20 (SBCMS-1387)
				if (tinymce.isOpera) {
					ed.onKeyDown.addToTop(function(ed, e) {
						if (((tinymce.isMac ? e.metaKey : e.ctrlKey) && e.keyCode == 86) || (e.shiftKey && e.keyCode == 45))
							grabContent(e);
...
(function(tinymce) {
	...
	//tinymce.ThemeManager.requireLangPack('sbcms');

	tinymce.create('tinymce.themes.SBCMSTheme', {
        ...
        renderUI : function(o) {
			...
			n = sc = DOM.add(n, 'table', {id : ed.id + '_tbl', cellPadding : 0, cellSpacing : 0, 'class' : 'mceLayout'});
			n = tb = DOM.add(n, 'tbody');

			// Create iframe container
			n = DOM.add(tb, 'tr');
			n = ic = DOM.add(DOM.add(n, 'td'), 'div', {'class' : 'mceIframeContainer'});
            //n = ic = DOM.add('div', {'class' : 'mceIframeContainer'});
	...
	tinymce.ThemeManager.add('sbcms', tinymce.themes.SBCMSTheme);
}(tinymce));
