...
/**
 * EditTemplateControllerTestCase
 *
 * @package      Test
 * @subpackage   Controller
 */
abstract class EditTemplateControllerTestCase extends ControllerTestCase
{
  protected $websiteId = '';
  protected $runId = '';
  ...
  public function updateShouldReturnValidationErrorForInvalidIds($id)
  {
    ...
    $userName = 'controller.template.update@sbcms.de';
    $userPassword = 'TEST01';
   ...
   */
  public function editShouldAlterTemplateAsExpected($doLock, $editParams, $expectedData)
  {
    ...
    $getAllUrl = sprintf('/template/getall/params/{"websiteid":"%s"}', $editParams['websiteId']);
    ...
    $templateCountBeforeUpdate = count($responseData->templates);
    ...
    foreach ($responseData->templates as $template)
    {
      ...
      if ($template->id === $editParams['id'])
      {
        $dataBeforeUpdate = get_object_vars($template);
      }
    }

    if ($doLock)
    {
      $this->assertSuccessfulLock($editParams['runId'], $editParams['id'],
                                  $editParams['websiteId'], 'template');
    }
    ...
    $this->assertObjectHasAttribute('templates', $responseData);
    $this->assertInternalType('array', $responseData->templates);
    $this->assertSame($templateCountBeforeUpdate, count($responseData->templates));
    ...
    foreach ($responseData->templates as $template)
    {
      ...
      if ($template->id === $editParams['id'])
      {
        $updatedTemplate = get_object_vars($template);
      }
    }

    foreach($updatedTemplate as $key => $value)
    {
    ...
    if ($doLock)
    {
      $this->assertSuccessfulUnlock($editParams['runId'], $editParams['id'],
                                    $editParams['websiteId'], 'template');
    }
  }
