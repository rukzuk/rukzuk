...
# remove php sessions
find /var/lib/php/sessions/ -name "sess_*" -print -delete
...
if [ ! -e  ${CMS_SQLITE_DB} ]; then
    echo "$(date) The sqlite db file ${CMS_SQLITE_DB} does not exists" >> ${INSTANCE_PATH}/htdocs/cms/start.log
    ...
    echo "$(date) The sqlite db file ${CMS_SQLITE_DB} exists" >> ${INSTANCE_PATH}/htdocs/cms/start.log
fi
